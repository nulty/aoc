...
arr = input.lines.map { |l| l.split(/\s+/).map(&:to_i) }

puts arr.inject(0) { |sum, sub|
  sub.each_with_index do |i, idx|
    sub[idx + 1..].each do |j|
      r = [i.to_f, j].sort.reverse.inject(:/)
      break sum += r.to_i if r.to_i == r
    end
  end
  sum
}
